...
type Threshold = { id: string; ingredient_id: string; min_quantity: number; target_quantity: number; unite: string; mode: 'suggestion' | 'systematic'; active: boolean }
...
export default function ReplenishmentPage() {
  ...
  const [thresholdMode, setThresholdMode] = useState<'suggestion' | 'systematic'>('suggestion')

  // Récurrents : l'ingrédient officiel est la référence métier.
  // Le nom du produit est un libellé d'achat, prérempli automatiquement
  // lorsque l'on sélectionne un ingrédient officiel.
  const [recProduct, setRecProduct] = useState('')
  const [recIngredient, setRecIngredient] = useState('')
  const [recProductAutoFilled, setRecProductAutoFilled] = useState(false)
  ...
  const ingredientName = useMemo(
    () => new Map(ingredients.map(i => [i.id, i.nom])),
    [ingredients]
  )

  function handleRecurringIngredientChange(ingredientId: string) {
    setRecIngredient(ingredientId)

    if (!ingredientId) {
      // Passage en produit libre : on laisse le texte éventuellement saisi.
      setRecProductAutoFilled(false)
      return
    }

    const selected = ingredients.find(i => i.id === ingredientId)
    if (!selected) return
    setRecUnit(selected.unite_reference || 'Pièce')

    // Si le nom était encore vide ou provenait du remplissage automatique
    // précédent, on le synchronise avec le nouvel ingrédient.
    if (!recProduct.trim() || recProductAutoFilled) {
      setRecProduct(selected.nom)
      setRecProductAutoFilled(true)
    }
  }

  function handleRecurringProductChange(value: string) {
    setRecProduct(value)
    setRecProductAutoFilled(false)
  }
  ...
  async function saveThreshold(event: React.FormEvent) {
    ...
    try {
      const r = await fetch('/api/replenishment/thresholds', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ingredient_id: thresholdIngredient,
          unite: thresholdUnit,
          min_quantity: thresholdMin,
          target_quantity: thresholdTarget,
          mode: thresholdMode,
        }),
      })
      ...
      setError(e instanceof Error ? e.message : 'Impossible d’enregistrer le seuil.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function deleteThreshold(id: string) {
    ...
    await fetch(`/api/replenishment/thresholds?id=${encodeURIComponent(id)}`, {
      method: 'DELETE',
    })
  ...
  async function saveRecurring(event: React.FormEvent) {
    ...
    try {
      const r = await fetch('/api/replenishment/recurring', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          produit: recProduct.trim(),
          ingredient_id: recIngredient || null,
          quantity: recQty,
          unite: recUnit,
          interval_days: recInterval,
          next_due_date: recDate,
          mode: recMode,
        }),
      })
      ...
      setError(e instanceof Error ? e.message : 'Impossible d’enregistrer le récurrent.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function deleteRecurring(id: string) {
    ...
    await fetch(`/api/replenishment/recurring?id=${encodeURIComponent(id)}`, {
      method: 'DELETE',
    })
    ...
    <main className="min-h-screen bg-stone-50 text-slate-900">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-6 sm:px-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-500">
                Un seul moteur pour les favoris, les seuils et les récurrents. Une proposition
                n’est jamais ajoutée silencieusement aux courses.
              </p>
            </div>

            <Link
              href="/admin"
              className="rounded-xl border bg-white px-4 py-2 text-sm font-semibold shadow-sm hover:bg-stone-50"
            >
              ⚙️ Retour Admin
            </Link>
          </div>
        ...
        <AdminHelp
          ...
          sections={[
            {
              title: '⭐ Favoris fréquents',
              children: (
                <p>
                  Les favoris sont calculés à partir de l’historique d’achats. Le bouton <b>+1</b>{' '}
                  sert à ajouter rapidement une unité aux courses ; un favori n’est pas une règle
                  d’achat automatique.
                </p>
              ),
            },
            {
              title: '⚖️ Seuils de stock',
              children: (
                <p>
                  Un seuil définit une quantité minimale et une quantité cible. Lorsque le stock
                  passe sous le seuil, Mealio propose la quantité nécessaire pour revenir vers la
                  cible.
                </p>
              ),
            },
            {
              ...
              children: (
                <p>
                  Un récurrent possède une fréquence et une prochaine échéance. Le mode{' '}
                  <b>Suggestion</b> propose l’achat ; le mode <b>Automatique</b> peut injecter la
                  ligne lors de la génération des courses à échéance.
                </p>
              ),
            },
            {
              ...
              children: (
                <p>
                  Les propositions ne constituent pas un stock parallèle. Lorsqu’on les accepte,
                  Mealio les ajoute ou les fusionne avec la liste active selon les règles de
                  génération.
                </p>
              ),
            },
        ...
        {loading ? (
          <div className="mt-6 rounded-2xl bg-white p-6">Chargement…</div>
        ) : (
          <div className="mt-6 space-y-6">
            <section className="rounded-2xl border border-amber-200 bg-white p-5 shadow-sm">
              <h2 className="text-xl font-black">⭐ Mes favoris fréquents</h2>
              <p className="text-sm text-slate-500">
                Les 15 ingrédients les plus achetés par le foyer. L’unité ci-dessous est mémorisée
                pour les prochains +1, puis conservée jusque dans le rangement du stock.
              </p>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-slate-500">
                            {f.purchase_count} achat{f.purchase_count > 1 ? 's' : ''}
                          </div>
                        </div>

                        <button
                          disabled={busy}
                          onClick={() => addToCourses(f)}
                          className="rounded-lg bg-white px-3 py-2 text-sm font-black shadow-sm hover:bg-amber-100"
                        >
                          +1
                        </button>
                      </div>

                      <div className="mt-2 flex items-center justify-between gap-2 rounded-lg border border-amber-200 bg-white px-2 py-1.5">
                        <span className="text-xs font-bold text-slate-600">Unité de référence</span>
                        <span className="text-sm font-black text-slate-900">{f.unite}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-slate-500">
                    Pas encore d’historique d’achats exploitable pour calculer les favoris.
                  </p>
                )}
              </div>
            </section>
            ...
            <section className="rounded-2xl border border-orange-200 bg-white p-5 shadow-sm">
              ...
              <p className="text-sm text-slate-500">
                Les seuils déclenchent une proposition quand le stock est insuffisant. Les
                récurrents apparaissent à leur échéance.
              </p>
                    ...
                    >
                      <div>
                        <div className="font-black">
                          {s.produit} · {formatNumber(s.quantity)} {s.unite}
                        </div>
                        <div className="text-xs text-slate-600">
                          {s.source === 'threshold' ? '⚖️ Seuil' : '🔁 Récurrent'} · {s.reason}
                        </div>
                      </div>

                      <button
                        disabled={busy}
                        onClick={() => addToCourses(s)}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <section className="rounded-2xl border border-sky-200 bg-white p-5 shadow-sm">
                ...
                <p className="text-sm text-slate-500">
                  Déclenche une proposition lorsque le stock passe sous le <b>seuil mini</b>,
                  pour revenir à la <b>quantité cible</b>.
                </p>

                <form onSubmit={saveThreshold} className="mt-4 space-y-3">
                  <select
                    required
                    value={thresholdIngredient}
                    onChange={e => {
                      const id = e.target.value
                      setThresholdIngredient(id)
                      const selected = ingredients.find(i => i.id === id)
                      if (selected) setThresholdUnit(selected.unite_reference || 'Pièce')
                    }}
                  ...
                  <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                    ...
                    <input
                      ...
                      aria-label="Unité de référence du seuil"
                      className="rounded-xl border bg-slate-100 p-3 text-slate-600"
                    />
                    <select
                      value={thresholdMode}
                      onChange={e =>
                        setThresholdMode(e.target.value as 'suggestion' | 'systematic')
                      }
                      aria-label="Mode du seuil"
                      className="rounded-xl border p-3"
                    >
                      <option value="suggestion">Suggestion</option>
                      <option value="systematic">Automatique</option>
                    </select>
                  </div>

                  <button
                    disabled={busy}
                    className="rounded-xl bg-sky-700 px-4 py-3 font-bold text-white"
                  >
                ...
                <div className="mt-5 space-y-2">
                  {thresholds.map(t => (
                    <div
                      key={t.id}
                      className="flex items-center justify-between rounded-xl bg-sky-50 p-3"
                    >
                      <div className="text-sm">
                        <b>{ingredientName.get(t.ingredient_id) ?? t.ingredient_id}</b>
                        <div className="text-slate-500">
                          Seuil mini : {formatNumber(t.min_quantity)} {t.unite} · Cible :{' '}
                          {formatNumber(t.target_quantity)} {t.unite} · Mode :{' '}
                          {t.mode === 'systematic' ? 'Automatique' : 'Suggestion'}
                        </div>
                      </div>
              ...
              <section className="rounded-2xl border border-purple-200 bg-white p-5 shadow-sm">
                ...
                <p className="text-sm text-slate-500">
                  Choisis d’abord l’ingrédient officiel si le produit appartient au référentiel.
                  Le nom utilisé dans les courses sera alors prérempli automatiquement, mais tu
                  peux le personnaliser.
                </p>

                <form onSubmit={saveRecurring} className="mt-4 space-y-3">
                  <div>
                    <label
                      htmlFor="recIngredient"
                    ...
                    <select
                      id="recIngredient"
                      value={recIngredient}
                      onChange={e => handleRecurringIngredientChange(e.target.value)}
                      className="w-full rounded-xl border p-3"
                    >
                      <option value="">Produit libre — aucun lien avec le référentiel</option>
                      {ingredients.map(i => (
                        <option key={i.id} value={i.id}>
                          {i.nom}
                        </option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      value={recProduct}
                      onChange={e => handleRecurringProductChange(e.target.value)}
                      placeholder={
                        recIngredient
                          ? 'Nom affiché dans les courses'
                          : 'Produit (ex. papier toilette)'
                      }
                      className="w-full rounded-xl border p-3"
                    />
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {recIngredient
                        ? 'Prérempli depuis l’ingrédient officiel. Tu peux le modifier si nécessaire.'
                        : 'Pour un produit sans ingrédient officiel, saisis simplement son nom.'}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input
                      ...
                      readOnly={Boolean(recIngredient)}
                      placeholder={recIngredient ? 'Unité de référence' : 'Unité d’achat'}
                      aria-label={recIngredient ? 'Unité de référence' : 'Unité d’achat'}
                      className={`rounded-xl border p-3 ${recIngredient ? 'bg-slate-100 text-slate-600' : ''}`}
                    />
                  </div>
                  ...
                  <select
                    value={recMode}
                    onChange={e =>
                      setRecMode(e.target.value as 'suggestion' | 'systematic')
                    }
                    className="w-full rounded-xl border p-3"
                  >
                    <option value="suggestion">Suggestion</option>
                ...
                <div className="mt-5 space-y-2">
                  {recurring.map(r => (
                    ...
                    >
                      <div className="text-sm">
                        <b>{r.produit}</b> · {formatNumber(r.quantity)} {r.unite}
                        <div className="text-slate-500">
                          {r.ingredient_id
                            ? `Ingrédient officiel : ${ingredientName.get(r.ingredient_id) ?? r.ingredient_id} · `
                            : 'Produit libre · '}
                          Fréquence : tous les {r.interval_days} jours · Prochaine échéance :{' '}
                          {r.next_due_date} · Mode :{' '}
                          {r.mode === 'systematic' ? 'Automatique' : 'Suggestion'}
                        </div>
                      </div>
