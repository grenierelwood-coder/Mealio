...
async function resolveOfficialIngredient(
  ...
  // 6. Aucun candidat : on mémorise l'absence de solution pour éviter
  // de rappeler Claude à chaque lancement.
  if (!candidates.length) {
    ...
    if (trace) {
      ...
      trace.events.push('Aucun candidat officiel : aucune consultation Claude')
    }
...
function convertStockQuantity(
  ...
  // Pièce <-> poids via un poids de référence propre à l'ingrédient.
  // La donnée est stockée dans ingredient_densities avec unite = 'Pièce'
  // et poids_g_approx = poids moyen d'une pièce en grammes.
  // Sans donnée explicite, la conversion reste impossible.
  if (ingredientId) {
    const pieceWeight =
      findDensity(refData, ingredientId, 'pièce') ??
      findDensity(refData, ingredientId, 'piece')

    if (pieceWeight) {
      const gramsPerPiece = Number(pieceWeight.poids_g_approx)

      if (Number.isFinite(gramsPerPiece) && gramsPerPiece > 0) {
        // Pièce -> poids
        if (from.type === 'unité' && target.type === 'poids') {
          const pieces = qty * from.factor
          const grams = pieces * gramsPerPiece

          return {
            qty: grams / target.factor,
            unit: target.unit,
            converted: true,
            reason: `${fromUnit} → ${targetUnit} via poids de référence (${gramsPerPiece} g/pièce)`,
          }
        }

        // Poids -> pièce
        if (from.type === 'poids' && target.type === 'unité') {
          const grams = qty * from.factor
          const pieces = grams / gramsPerPiece

          return {
            qty: pieces / target.factor,
            unit: target.unit,
            converted: true,
            reason: `${fromUnit} → ${targetUnit} via poids de référence (${gramsPerPiece} g/pièce)`,
          }
