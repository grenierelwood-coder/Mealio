...
if (bestIsSafe(candidates, CONFIG.OFFICIAL_AUTO_SCORE, CONFIG.OFFICIAL_AUTO_MARGIN)) {
  ...
  // Elle est consultée avant le lexical ambigu : une décision déjà apprise reste prioritaire.
  ...
  if (previous) {
    ...
    if (previous.statut === 'rejete') {
      // Un rejet mémorisé doit forcer une nouvelle résolution déterministe/IA uniquement
      // si une nouvelle proposition devient possible ; on ne réutilise pas l'ancienne.
...
function getUnitMapping(refData: ReferenceData, unit: string) {
  ...
  return refData.unitMappings.find(
    u =>
      cleanText(u.unite) === cleaned ||
      cleanText(u.abreviation ?? '') === cleaned
  )
}
...
function canonicalUnit(refData: ReferenceData, unit: string): {
  ...
  if (cleaned === 'g' || cleaned === 'gramme') {
    return { unit: 'g', factor: 1, type: 'poids' }
  }
  if (cleaned === 'ml' || cleaned === 'millilitre') {
    return { unit: 'mL', factor: 1, type: 'volume' }
  }
  if (cleaned === 'piece' || cleaned === 'unite') {
    return { unit: 'pièce', factor: 1, type: 'unité' }
  }

  const mapping = getUnitMapping(refData, unit)
  if (!mapping) return null

  const type = mapping.type_unite
  const factor = Number(mapping.multiplicateur ?? 1)
